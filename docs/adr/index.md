...
| [0031 · The id names are `NewType`s](0031-the-id-names-are-newtypes.md) | `EntityId`, `RelationshipId`, `TenantId` and `SourceId` become distinct to a type checker and identical at runtime, so transposing `(entity_id, tenant_id)` is a gate failure. Amends 0002 and 0006 in typing only; 0001 needed nothing, because `NewType` has no wire representation. |
