...
#: `kg_builder.domain.vector.cosine_score` rather than merely their order.
...
class PgVectorStore:
    ...
    def _insert_sql(self) -> str:
        ...
        """
        return (
            f"INSERT INTO {self._table} (tenant_id, entity_id, embedding, entity_type, metadata) "
            "SELECT * FROM unnest("
    ...
    async def get(self, entity_id: EntityId, tenant_id: TenantId) -> VectorRecord | None:
        row = await self._pool.fetchrow(
            ...
            f"SELECT embedding::real[] AS vector, metadata FROM {self._table} "
    ...
    def _search_sql(self) -> str:
        ...
        """
        return (
            f"SELECT entity_id, metadata, {_SCORE} AS score "
            f"FROM {self._table} "
            "WHERE tenant_id = $1 "
    ...
    async def delete(self, entity_id: EntityId, tenant_id: TenantId) -> bool:
        deleted = await self._pool.fetchval(
            f"DELETE FROM {self._table} WHERE tenant_id = $1 AND entity_id = $2 RETURNING 1",
            tenant_id,
            entity_id,
        )
    ...
    async def delete_by_tenant(self, tenant_id: TenantId) -> int:
        ...
        removed = await self._pool.fetchval(
            f"WITH removed AS (DELETE FROM {self._table} "
