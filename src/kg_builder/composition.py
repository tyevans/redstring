...
which is why `kg_builder.__init__` exported a version string and nothing else.
...
`kg_builder.projections.project` over the feed. `report.event` is returned for
...
from kg_builder.extraction.pipeline import DEFAULT_SYSTEM_PROMPT, ExtractionPipeline
