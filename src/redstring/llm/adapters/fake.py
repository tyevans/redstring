...
class FakeLlmProvider:
    ...
    def _next_response(self, text: str) -> Response:
        if self._script is not None:
            ...
            # `raise AssertionError` rather than `assert`: this is a defect in
            # the test and not a condition the library can be in, so
            # AssertionError is the right type -- but `assert` is stripped
            # under `python -O`, and a fake that silently IndexErrors in an
            # optimised interpreter reports the wrong bug. Same reasoning for
            # the narrowing check below.
        ...
        if self._by_substring is None:  # pragma: no cover - constructor invariant
            raise AssertionError("FakeLlmProvider has neither a script nor a substring map")
