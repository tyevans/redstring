...
class ExtractionRetryPolicy:
    ...
    def __init__(
        ...
        # module's tests had to poison `sys.modules["redstring.config"]`
    ...
    def get_delay(self, attempt: int) -> float:
        ...
        #
        # `random` rather than `secrets` (bandit B311, suppressed): jitter
        # exists to decorrelate retries between clients so a recovering server
        # is not hit by a synchronised thundering herd. An attacker who can
        # predict the delay learns when the next request arrives and nothing
        # else -- there is no secret here, and a CSPRNG would cost entropy per
        # retry to defend a property nobody depends on.
        if self.jitter > 0:
            jitter_range = base_delay * self.jitter
            jittered_delay = base_delay + random.uniform(-jitter_range, jitter_range)  # nosec B311
