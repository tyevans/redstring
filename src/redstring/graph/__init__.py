...
Sibling of `redstring.vector` in the layered contract: each holds the
...
composition root (`redstring.graph.adapters.neo4j`,
`redstring.graph.adapters.memory`); a package-level re-export would make
`import redstring.graph` pull the `neo4j` driver in, and that is an optional
extra.
"""
