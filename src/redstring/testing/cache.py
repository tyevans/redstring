...
class CacheCompliance:
    ...
    class TestKeyValue:
        async def test_a_missing_key_is_none_not_an_error(self, cache: Cache) -> None:
            assert await cache.get("nothing here") is None

        async def test_what_was_set_comes_back(self, cache: Cache) -> None:
            await cache.set("state", "open")

            assert await cache.get("state") == "open"

        async def test_a_value_comes_back_as_str_not_bytes(self, cache: Cache) -> None:
        ...
        async def test_setting_again_replaces(self, cache: Cache) -> None:
            await cache.set("state", "open")
            await cache.set("state", "closed")

            assert await cache.get("state") == "closed"

        async def test_delete_removes_the_key(self, cache: Cache) -> None:
            await cache.set("state", "open")
            await cache.delete("state")

            assert await cache.get("state") is None

        async def test_deleting_a_key_that_is_not_there_is_not_an_error(self, cache: Cache) -> None:
            await cache.delete("never existed")

    class TestCounters:
        async def test_the_first_increment_returns_one(self, cache: Cache) -> None:
            """A missing counter is zero, so the first hit is 1, not None or 0."""
            assert await cache.increment("failures") == 1

        async def test_increments_accumulate(self, cache: Cache) -> None:
            for expected in (1, 2, 3):
                assert await cache.increment("failures") == expected

        async def test_a_counter_reads_back_as_its_decimal_string(self, cache: Cache) -> None:
        ...
        async def test_a_deleted_counter_starts_again_from_one(self, cache: Cache) -> None:
            await cache.increment("failures")
            await cache.delete("failures")

            assert await cache.increment("failures") == 1

        async def test_a_counter_set_directly_continues_from_there(self, cache: Cache) -> None:
    ...
    class TestHitWindows:
        ...
        async def test_a_hit_exactly_at_the_boundary_is_inside_the_window(
            self, cache: Cache
        ) -> None:
        ...
        async def test_two_hits_at_the_very_same_instant_are_two_hits(self, cache: Cache) -> None:
        ...
        async def test_the_oldest_hit_in_the_window_is_reported(self, cache: Cache) -> None:
        ...
        async def test_the_oldest_hit_ignores_hits_that_have_aged_out(self, cache: Cache) -> None:
            await cache.record_hit("tenant", at=NOW - 600, ttl_seconds=3600)
            await cache.record_hit("tenant", at=NOW, ttl_seconds=3600)

            assert await cache.oldest_hit("tenant", since=NOW - 60) == pytest.approx(NOW)

        async def test_windows_are_kept_apart_by_key(self, cache: Cache) -> None:
            await cache.record_hit("tenant-a", at=NOW, ttl_seconds=300)

            assert await cache.count_hits("tenant-b", since=NOW - 60) == 0

        async def test_a_window_and_a_value_can_share_a_key_without_colliding(
            self, cache: Cache
        ) -> None:
        ...
        async def test_deleting_a_key_clears_its_window_too(self, cache: Cache) -> None:
            await cache.record_hit("tenant", at=NOW, ttl_seconds=300)
    ...
    class TestExpiry:
        async def test_a_value_with_a_ttl_is_gone_once_it_elapses(self, cache: Cache) -> None:
        ...
        async def test_a_value_without_a_ttl_survives(self, cache: Cache) -> None:
            import asyncio
        ...
        async def test_a_counters_ttl_is_not_refreshed_by_later_increments(
            self, cache: Cache
        ) -> None:
    ...
    class TestLifecycle:
        async def test_close_is_safe_to_call_twice(self, cache: Cache) -> None:
            await cache.close()
            await cache.close()
