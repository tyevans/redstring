...
### B163. A `LEXICAL`-only retrieval still requires an `EmbeddingProvider` and a `VectorReader`

`Retriever.__init__` takes `embeddings` and `vectors`, checks their dimensions
against each other, and stores both — before any mode has been chosen.
`retrieve` then branches per mode and `RetrievalMode.LEXICAL` reaches neither:
it calls `_lexical`, which is `find_by_blocking_keys` plus `lexical_score`
over the entity's name, with no vector anywhere in the path.

So a caller who wants only the blocking-key channel must still supply, wire and
keep healthy an embedding endpoint it never calls. There is no constructor that
expresses "lexical only", and `mode=` cannot help because the objects are
already required by then.

**Found by a consumer, not by inspection.** `research-team` adopted `Retriever`
for its entity-search tool and chose `LEXICAL` deliberately — fusing the
semantic channel in made the tool answer name lookups with entities matching
the query nowhere in their text, which its own tests caught. Its embedding
probe latches `(None, None)` when the endpoint is absent or misconfigured, a
deliberate degradation for consolidation scoring. The result is that switching
embeddings off, or mistyping a model name, silently removes *misspelling-
tolerant entity search* — a feature with no embedding in it — and leaves a
plain substring scan behind.

**The local workaround is closed off, which is why this is filed here.**
Assembling a lexical-only search the way a caller can for chunks — redstring
exports `tokenize` and `rank_chunks`, so `ChunkRetriever`'s lexical half is
reproducible without an `EmbeddingProvider` — is not possible for entities:
`query_blocking_keys`, `blocking_keys_for` and `lexical_score` all live in
`redstring.domain.blocking` and none is exported from the package root. The
entity side and the chunk side are asymmetric here, and nothing records that
as a decision.

**Two shapes, and they are not equivalent:**

1. Make `embeddings` and `vectors` optional on `Retriever`, and raise from
   `retrieve` when a mode that needs them is asked for with them absent. Keeps
   one class and one entry point; moves a construction-time error to call time
   for the semantic modes, which ADR 0017's reasoning about surfacing
   configuration mistakes at the seam argues against.
2. Export the blocking-key helpers, so a caller can assemble a lexical-only
   entity search the way it already can for chunks. Adds three names to the
   public surface and makes the two retrieval paths symmetric, at the cost of
   pinning the blocking scheme as public API — which consolidation also depends
   on, so it would then be harder to change.

(1) is smaller. (2) is the one that removes the asymmetry rather than working
around it. Not chosen here: the trade is about how much of the blocking scheme
should be public, which is an ADR-sized question and belongs with whoever
answers it.

Not urgent for the consumer that found it: its default configuration has an
embedding provider, so the gap bites the deployment that switched embeddings
off deliberately — which is also the one least likely to be watching for a
quiet loss of capability.
